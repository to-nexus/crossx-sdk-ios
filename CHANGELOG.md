...
### Changed
- **`recoverMigratedWallet(recoveryPin:)`** — Migration UI(Wallet Found 프롬프트, PIN 입력 모달) 제거, 복구 PIN을 호출자가 직접 전달하도록 시그니처 변경
  - 내부에서 `checkWallet()` + `verifyRecoveryPin` API 직접 호출
  - `migrationPresenter` 의존성 제거
...
### Fixed
- **`CrossWebAuthKit` 의존성 고정** — `Package.swift`에서 `branch: "master"` 참조를 `exact: "1.0.0"`으로 변경하여 빌드 재현성 보장
